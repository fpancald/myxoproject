...
xleft = linspace(0,L/2,xsteps);   %space domain left
xright = linspace(L/2,L,xsteps);   %space domain right
...
sol = pdepe(m,@pdex1pde,@pdex1ic,@pdex1bc,x,t);%before division
...
u1pdl = solpdl(:,:,1);%MinD-like free
u2pdl = solpdl(:,:,2);%bounded
u3pdl = solpdl(:,:,3);%MinE-like free
u4pdl = solpdl(:,:,4);%bounded
u5pdl = solpdl(:,:,5);%RomR free
u6pdl = solpdl(:,:,6);%bounded

u1pdr = solpdr(:,:,1);%MinD-like free
u2pdr = solpdr(:,:,2);%bounded
u3pdr = solpdr(:,:,3);%MinE-like free
u4pdr = solpdr(:,:,4);%bounded
...
if t>divT && Div==1 && Diff==1 && x>(L-rpol)/2 && x<(L+rpol)/2 %sp. diffusion and division ON near center
    ...
    DDtx=(abcd(1)*x^3+abcd(2)*x^2+abcd(3)*x+abcd(4))*DD+DD;
    DEtx=(abcd(1)*x^3+abcd(2)*x^2+abcd(3)*x+abcd(4))*DE+DE;
    DRtx=(abcd(1)*x^3+abcd(2)*x^2+abcd(3)*x+abcd(4))*DR+DR;
    DDtxDx=(3*abcd(1)*x^2+2*abcd(2)*x+abcd(3))*DD;
    DEtxDx=(3*abcd(1)*x^2+2*abcd(2)*x+abcd(3))*DE;
    DRtxDx=(3*abcd(1)*x^2+2*abcd(2)*x+abcd(3))*DR;
else
    DDtx=DD;
    DEtx=DE;
    DRtx=DR;
...
elseif x>(L-rpol)/2 && x<(L+rpol)/2 && t>T %after division near separation (new poles)
    
    if x<L/2%left of center
        
    %     ab=deg1interpol((L-rpol)/2,L/2,0,N0/2*(1+u(4)/(4*icE))*(t-divT)/(T-divT));
    %     N=ab(1)*x+ab(2);
        abcd=abcdlc*N0/2*(1+u(4)/(4*icE));
        N=abcd(1)*x^3+abcd(2)*x^2+abcd(3)*x+abcd(4);
    ...
    %     ab=deg1interpol((L-rpol)/2,L/2,0,N0/2*(1+u(4)/(4*icE))*(t-divT)/(T-divT));
    %     N=ab(1)*x+ab(2);
        abcd=abcdrc*N0/2*(1+u(4)/(4*icE));
        N=abcd(1)*x^3+abcd(2)*x^2+abcd(3)*x+abcd(4);
    end
    
else%not close to center,not poles or close
    N=0;
end
...
c = [1;1;1;1;1;1]; 
f = [DDtx;1e-10;DEtx;1e-10;DRtx;1e-10] .* DuDx; 
